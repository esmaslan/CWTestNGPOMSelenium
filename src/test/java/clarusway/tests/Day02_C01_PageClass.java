...
public class Day02_C01_PageClass {
    ...
    public void test() {
        ...
        WebElement usernmae = Driver.getDriver().findElement(By.id("user-name"));
        usernmae.sendKeys("standard_user");

        WebElement password = Driver.getDriver().findElement(By.id("password"));
        password.sendKeys("secret_sauce");

        WebElement loginBtn = Driver.getDriver().findElement(By.id("login-button"));
        loginBtn.click();

        Select drpodown = new Select(Driver.getDriver().findElement(By.tagName("select")));
        drpodown.selectByValue("lohi");
        ...
        List<Double> urunFiyatlari = itemPrices.stream().
                map(x -> x.getText().substring(1)).map(x -> Double.parseDouble(x)).
